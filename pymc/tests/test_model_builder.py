import pymc as pm 
from model_builder import ModelBuilder

#relevant libraries
import arviz
import pandas as pd
from pathlib import Path
import numpy as np
from typing import Dict
import cloudpickle
import arviz as az
import hashlib

class LinearModel(ModelBuilder):
    '''
    Simple linear model.
    '''
    _model_type = 'LinearModel'
    version = '0.1'

    def _build(self):
        '''
        Build the model.
        Requires suitable self.data and self.model_config.
        '''
        # data
        x = pm.MutableData('x', self.data['input'].values)
        y_data = pm.MutableData('y_data', self.data['output'].values)

        # prior parameters
        a_loc = self.model_config['a_loc']
        a_scale = self.model_config['a_scale']
        b_loc = self.model_config['b_loc']
        b_scale = self.model_config['b_scale']
        obs_error = self.model_config['obs_error']

        # priors
        a = pm.Normal("a", a_loc, sigma=a_scale)
        b = pm.Normal("b", b_loc, sigma=b_scale)
        obs_error = pm.HalfNormal("σ_model_fmc", obs_error)

        # observed data
        y_model = pm.Normal('y_model', a + b * x, obs_error, observed=y_data)


    def _data_setter(self, data : pd.DataFrame):
        '''
        Sets new data in the model.
        To discuss: Is there a way without this method?
        '''
        with self.model:
            pm.set_data({'x': data['input'].values})
            try: # if y values in new data
                pm.set_data({'y_data': data['output'].values})
            except: # dummies otherwise
                pm.set_data({'y_data': np.zeros(len(data))})


    @classmethod
    def create_sample_input(cls):
        '''
        Example data structures for simple linear model
        '''
        x = np.linspace(start=0, stop=1, num=100)
        y = 5 * x + 3 + np.random.normal(0, 1, len(x))
        y_noise = y + np.random.normal(scale=.5, size=100)
        data = pd.DataFrame({'input': x, 'output': y_noise})

        model_config = {
            'a_loc': 7,
            'a_scale': 3,
            'b_loc': 5,
            'b_scale': 3,
            'obs_error': 2,
        }

        sampler_config = {
            'draws': 1_000,
            'tune': 1_000,
            'chains': 1,
            'target_accept': 0.95,
        }

        return data, model_config, sampler_config

def test_build():
	